...
namespace UnityEditor.XCodeEditor 
{
	public class XCMod 
	{
		...
		public string bundle_identifier {
			get {
				if (_datastore != null && _datastore.Contains("bundle_identifier"))
					return (string)_datastore["bundle_identifier"];
				return string.Empty;
			}
		}

		public string enable_ObjectC_Exceptions {
			get {
				if (_datastore != null && _datastore.Contains("enable _ObjectC_Exceptions"))
					return (string)_datastore["enable _ObjectC_Exceptions"];
				return string.Empty;
			}
		}
		public string enable_bitcode {
			get {
				if (_datastore != null && _datastore.Contains("enable_bicode"))
		...
		{	
			...
			{
				ArrayList argFile = this.files;
				if( null != argFile )
				{
					for( int lCnt = 0; lCnt < argFile.Count; ++lCnt )
					{
						argFile[lCnt] = path + (string)argFile[lCnt];
					}
				}
				
				ArrayList argFolders = this.folders;
				if( null != argFolders )
				{
					for( int lCnt = 0; lCnt < argFolders.Count; ++lCnt )
					{
						argFolders[lCnt] = path + (string)argFolders[lCnt];
					}
				}
