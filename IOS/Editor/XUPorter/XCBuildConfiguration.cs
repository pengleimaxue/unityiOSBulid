...
namespace UnityEditor.XCodeEditor
{
	public class XCBuildConfiguration : PBXObject
	{
		...
		protected const string ENABLE_OBJECT_EXCEPTIONS_KEY = "GCC_ENABLE_OBJC_EXCEPTIONS";	
		protected const string RODUCT_BUNDLE_IDENTIFIER_KEY  = "PRODUCT_BUNDLE_IDENTIFIER";
		...
		public bool AddBundleIdentifierFlags( string flag = " ")
		{
			bool modified = false;
			if( !ContainsKey( BUILDSETTINGS_KEY ) )
				this.Add( BUILDSETTINGS_KEY, new PBXSortedDictionary() );
			if( !((PBXDictionary)_data[BUILDSETTINGS_KEY]).ContainsKey(RODUCT_BUNDLE_IDENTIFIER_KEY) ) {
				((PBXDictionary)_data[BUILDSETTINGS_KEY]).Add(RODUCT_BUNDLE_IDENTIFIER_KEY , flag );
				modified = true;
			}
		...
		public bool AddEnableExceptionSettingFlags(string flag = "YES")
		{
			bool modified = false;
			
			if( !ContainsKey( BUILDSETTINGS_KEY ) )
				this.Add( BUILDSETTINGS_KEY, new PBXSortedDictionary() );
			if( !((PBXDictionary)_data[BUILDSETTINGS_KEY]).ContainsKey(ENABLE_OBJECT_EXCEPTIONS_KEY) ) {
				((PBXDictionary)_data[BUILDSETTINGS_KEY]).Add(ENABLE_OBJECT_EXCEPTIONS_KEY , flag );
				modified = true;
			}
			return modified;
		}
