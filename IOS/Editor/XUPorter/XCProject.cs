...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		#region PBXMOD
		public bool AddBundleIdentifierFlags(string flage) 
		{
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				buildConfig.Value.AddBundleIdentifierFlags(flage);
			}
			modified = true;
			return modified;	
		}
		public bool AddEnableExceptionSettingFlags(string flage) 
		{
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				buildConfig.Value.AddEnableExceptionSettingFlags(flage);
			}
			modified = true;
			return modified;	
		}
		public bool AddEnableBitcodeFlags(string flage) 
		{
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				buildConfig.Value.AddEnableBitcodeFlags(flage);
			}
			modified = true;
			return modified;
		}
		...
		{	
			...
			if(mod.enable_bitcode !=null) {
				this.AddEnableBitcodeFlags(mod.enable_bitcode);
			}

			if(mod.enable_ObjectC_Exceptions !=null) {
				this.AddEnableExceptionSettingFlags(mod.enable_ObjectC_Exceptions);
			}
			if(mod.bundle_identifier !=null) {
				this.AddBundleIdentifierFlags(mod.bundle_identifier);
			}
